...
<a id="rule-400-7b"></a>
400.7b Effects from static abilities that grant an ability to a permanent spell that functions on the battlefield continue to apply to the permanent that spell becomes (see rule [611.3d](../6-spells-abilities-and-effects/611-continuous-effects.md#rule-611-3d)).
...
<a id="rule-400-7f"></a>
400.7f Abilities that trigger when an enchanted permanent leaves the battlefield can find the new object that each Aura enchanting that permanent became in its owner’s graveyard if it was put into that graveyard at the same time the enchanted permanent left the battlefield. It can also find the new object that each Aura enchanting it became in its owner’s graveyard as a result of being put there as a state-based action for not being attached to a permanent. (See rule [704.5m](../7-additional-rules/704-state-based-actions.md#rule-704-5m).)
...
<a id="rule-400-7k"></a>
400.7k After resolving a madness triggered ability (see rule [702.35](../7-additional-rules/702-keyword-abilities.md#rule-702-35)), if the exiled card wasn’t cast and was moved to a public zone, effects referencing the discarded card can find that object.

<a id="rule-400-7m"></a>
400.7m Stickers on an object in a public zone are retained as it moves to another public zone (see rule [123.5](../1-game-concepts/123-stickers.md#rule-123-5)). Any effects from stickers continue to apply to the new object it becomes in that zone.
...
<a id="rule-400-11a"></a>
400.11a Cards in a player’s sideboard are outside the game. See rule [100.4](../1-game-concepts/100-general.md#rule-100-4).
...
<a id="rule-400-11c"></a>
400.11c Cards outside the game can’t be affected by spells or abilities, except for characteristic-defining abilities printed on them (see rule [604.3](../6-spells-abilities-and-effects/604-handling-static-abilities.md#rule-604-3)) and spells and abilities that allow those cards to be brought into the game.
