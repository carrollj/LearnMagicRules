...
<a id="rule-730-1"></a>
730.1. One keyword causes an object to merge with a permanent. See rule [702.140](702-keyword-abilities.md#rule-702-140), “Mutate.”
...
<a id="rule-730-2a"></a>
730.2a A merged permanent has only the characteristics of its topmost component, unless otherwise specified by the effect that caused them to merge. This is a copiable effect whose timestamp is the time the objects merged. (See rule [613.2](../6-spells-abilities-and-effects/613-interaction-of-continuous-effects.md#rule-613-2).)
...
<a id="rule-730-2h"></a>
730.2h If a merged permanent contains a flip card (see rule [710](710-flip-cards.md#rule-710)), that component’s alternative characteristics are used instead of its normal characteristics if the merged permanent is flipped.
...
<a id="rule-730-3b"></a>
730.3b If a player exiles a merged permanent, that player determines the relative timestamp order of the cards at that time. This is an exception to the procedure described in rule [613.7m](../6-spells-abilities-and-effects/613-interaction-of-continuous-effects.md#rule-613-7m).

<a id="rule-730-3c"></a>
730.3c If an effect can find the new object that a merged permanent becomes as it leaves the battlefield, it finds all of those objects. (See rule [400.7](../4-zones/400-general.md#rule-400-7).) If that effect causes actions to be taken upon those objects, the same actions are taken upon each of them.

<a id="rule-730-3d"></a>
730.3d If multiple replacement effects could be applied to the event of a merged permanent leaving the battlefield or being put into the new zone, applying one of those replacement effects to the object applies it to all components of the object. If the merged permanent is a commander, it may be exempt from this rule; see rules [903.9b](../9-casual-variants/903-commander.md#rule-903-9b)–c.
