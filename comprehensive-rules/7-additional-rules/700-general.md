...
<a id="rule-700-2a"></a>
700.2a The controller of a modal spell or activated ability chooses the mode(s) as part of casting that spell or activating that ability. If one of the modes would be illegal (due to an inability to choose legal targets, for example), that mode can’t be chosen. (See rule [601.2b](../6-spells-abilities-and-effects/601-casting-spells.md#rule-601-2b).)

<a id="rule-700-2b"></a>
700.2b The controller of a modal triggered ability chooses the mode(s) as part of putting that ability on the stack. If one of the modes would be illegal (due to an inability to choose legal targets, for example), that mode can’t be chosen. If no mode is chosen, the ability is removed from the stack. (See rule [603.3c](../6-spells-abilities-and-effects/603-handling-triggered-abilities.md#rule-603-3c).)
...
<a id="rule-700-2g"></a>
700.2g A copy of a modal spell or ability copies the mode(s) chosen for it. The controller of the copy can’t choose a different mode. (See rule [707.10](707-copying-objects.md#rule-707-10).)

<a id="rule-700-2h"></a>
700.2h Some modal spells have one or more modes with a cost listed before the effect of that mode. This indicates that the mode has an additional cost that must be paid as the spell is cast if that mode is chosen. If more than one such mode is chosen, all additional costs must be paid to cast that spell. Paying these costs follows the rules for paying additional costs in rules [601.2b](../6-spells-abilities-and-effects/601-casting-spells.md#rule-601-2b) and [601.2f](../6-spells-abilities-and-effects/601-casting-spells.md#rule-601-2f)–h.
...
<a id="rule-700-8d"></a>
700.8d One card, Stick Together, instructs players to choose a party from among creatures they control. To do so, for each of the creature types listed in rule [700.8](700-general.md#rule-700-8), each player chooses up to one creature they control of that type.

<a id="rule-700-9"></a>
700.9. Some cards refer to modified permanents. A permanent is modified if it has one or more counters on it (see rule [122](../1-game-concepts/122-counters.md#rule-122)), if it is equipped (see rule [301.5](../3-card-types/301-artifacts.md#rule-301-5)), or if it is enchanted by an Aura that is controlled by that permanent’s controller (see rule [303.4](../3-card-types/303-enchantments.md#rule-303-4)).
